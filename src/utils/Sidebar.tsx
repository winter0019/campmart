...
export default function Sidebar({
  ...
  // Dynamically map navigation access based on production roles
  const menuItems = [
        ...
        <div className="p-4 border-t border-slate-800 shrink-0 bg-slate-950/40">
          {showExpanded ? (
            <div className="space-y-4">
              {/* User Profile Info */}
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-300 font-bold uppercase overflow-hidden text-sm shrink-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    {user?.role === "admin" ? (
                      ...
                      <Smartphone className="w-3 h-3 text-teal-400 shrink-0" />
                    )}
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="w-9 h-9 rounded-full bg-slate-800 flex items-center justify-center text-slate-300 font-bold uppercase text-xs shrink-0">
                {user?.fullName?.charAt(0) || "U"}
              </div>
              <button
                onClick={onLogout}
                title="Logout session"
                className="p-2 text-red-400 hover:bg-slate-800 rounded-lg cursor-pointer transition-all flex items-center justify-center"
              >
